...
export class CryptoService {
  ...
  public generateKey( password: string, salt: string ): PromiseLike<Tuple<string, string>> {
    ...
    return window.crypto.subtle.importKey(
      ...
      ['deriveBits', 'deriveKey']
        ...
        true,
        ['encrypt', 'decrypt']
      ) ).then( ( webKey ) => crypto.subtle.exportKey( 'raw', webKey ) ).then( ( buffer ) => new Tuple( this.ab2str( buffer ), this.ab2str(<Uint8Array> saltBuffer) ) );
  }
  ...
  public encryptTextAes(password: string, salt: string, text: string): PromiseLike<string> {
    const saltBuffer = this.str2ab( salt );
    return Promise.all([this.keycreator(password, salt, text).then(  webKey  => Promise.all([webKey]).then(webKey2 => window.crypto.subtle.encrypt({ name: 'AES-CBC', iv: saltBuffer }, webKey2[0], new TextEncoder().encode(text)))
  ...
  public decryptTextAes(password: string, salt: string, text: string): PromiseLike<string> {
    const saltBuffer = this.str2ab( salt );
    return Promise.all([this.keycreator(password, salt, text).then(  webKey  => Promise.all([webKey]).then(webKey2 => window.crypto.subtle.decrypt({ name: 'AES-CBC', iv: saltBuffer }, webKey2[0], this.str2ab(text)))
