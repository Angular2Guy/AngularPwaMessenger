...
export class CryptoService {
  ...
  public generateKeys(password: string): PromiseLike<Tuple<string, string>> {
    return window.crypto.subtle
      ...
      .then((value) => this.createKeyTuple(password, value));
  }

  public encryptLargeText(
    msgText: string,
    keyStr: string
  ): PromiseLike<string> {
  ...
  public decryptLargeText(
    ...
    const blocks = chunks.map((chunk) =>
      this.decryptText(chunk, keyStr, keyPwd)
    );
    return Promise.all(blocks).then((myChunks) => myChunks.join(""));
  }
  ...
  public decryptText(
    ...
    const result = Promise.all([
      this.createWrapKey(keyPwd, keyJson.salt)
        .then((value) => {
          const algo1: AesGcmParams = {
            name: "AES-GCM",
            iv: this.str2ab(keyJson.salt),
          };
          const algo2: RsaHashedImportParams = {
            name: "RSA-OAEP",
            hash: "SHA-256",
          };
        ...
        .then((myValue) => {
          ...
          return Promise.all([myValue]).then((myValue2) =>
        ...
        .then((value2) =>
          Promise.all([value2]).then((value3) =>
            new TextDecoder().decode(value3[0])
          )
        ),
    ]).then((myResult) => myResult[0]);
    return result;
  }
  ...
  public generateKey(
    password: string,
    salt: string
  ): PromiseLike<Tuple<string, string>> {
    ...
    return window.crypto.subtle
      .importKey("raw", passphraseKey, "PBKDF2", false, [
        "deriveBits",
        "deriveKey",
      ])
      .then((key) =>
      ...
      .then(
        (buffer) =>
          new Tuple(this.ab2str(buffer), this.ab2str(saltBuffer as Uint8Array))
      );
  }

  public encryptTextAes(
    password: string,
    salt: string,
    text: string
  ): PromiseLike<string> {
    ...
    return Promise.all([
      this.keycreator(password, salt, text).then((webKey) =>
        Promise.all([webKey])
          .then((webKey2) =>
            window.crypto.subtle.encrypt(
              { name: "AES-CBC", iv: saltBuffer },
              webKey2[0],
              new TextEncoder().encode(text)
            )
          )
          .then((result) =>
            Promise.all([result]).then((myResult) => this.ab2str(myResult[0]))
          )
      ),
    ]).then((result2) => result2[0]);
  }

  public decryptTextAes(
    password: string,
    salt: string,
    text: string
  ): PromiseLike<string> {
        ...
        Promise.all([webKey])
          ...
          .then((result) =>
            Promise.all([result]).then((result2) =>
              new TextDecoder().decode(result2[0])
            )
          )
      ),
  ...
  public hashServerPW(password: string): PromiseLike<string> {
    return window.crypto.subtle
      .digest("SHA-384", new TextEncoder().encode(password))
      .then((value) => this.ab2str(value));
  }

  public createSecurePassword(): string {
    return this.ab2str(
      window.crypto.getRandomValues(new Uint8Array(32)) as Uint8Array
    )
      .replace("=", "")
      .replace(/[^a-z0-9]/gi, "");
  }
  ...
  private createKeyTuple(
    password: string,
    cryptoKeyPair: CryptoKeyPair
  ): PromiseLike<Tuple<string, string>> {
    let jwkPubKey: string = null;
    let wrapKey: CryptoKey = null;
    const mySalt = this.ab2str(
      window.crypto.getRandomValues(new Uint8Array(16)) as Uint8Array
    );
    return window.crypto.subtle
      .exportKey("jwk", cryptoKeyPair.publicKey)
      .then((value) => {
