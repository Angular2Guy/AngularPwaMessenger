...
export class LoginComponent implements OnInit {
  ...
  onSigninClick(): void {
    ...
    this.cryptoService
      .hashPW(this.signinForm.get(FormFields.password).value)
      .then((value) => this.cryptoService.generateKeys(value))
      .then((result) => {
        myUser.privateKey = result.b;
        myUser.publicKey = result.a;
      })
      .then(() =>
        this.cryptoService.hashServerPW(
          this.signinForm.get(FormFields.password).value
        )
      )
  ...
  login(us: MyUser, localUser: LocalUser): void {
    this.cryptoService
      .generateKey(us.password, localUser.salt)
      .then((tuple) => {
        if (
          (us.username !== null || localUser.username !== null) &&
          localUser.hash === tuple.a
        ) {
          if (us.token) {
  ...
  private createLocalUser(us: MyUser, passwd: string): PromiseLike<LocalUser> {
    ...
    return this.cryptoService
      .generateKey(passwd, us.salt ? us.salt : null)
      .then((result) => {
        localUser = {
